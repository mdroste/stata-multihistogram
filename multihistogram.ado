...
program define multihistogram, rclass
	version 15
	set more off
	syntax varlist(max=4 numeric) [if] [in], [twopt(string asis) start(real -10) bin(integer -10) opacity(integer 40) if1(string asis) if2(string asis) if3(string asis) if4(string asis)]
	...
	*---------------------------------
	...
	foreach v in `varlist' {
	    local twoway_cmd `twoway_cmd' (histogram `v' `fif`curr_var'', start(`min_val') width(`width') color(`colors`curr_var''%`opacity'))
